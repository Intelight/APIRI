...
/* Frame 80: SIU/CMU Module Description Request */
static FIOMSG_TX_FRAME frame_80_init =
{
	.def_freq	= FIO_HZ_ONCE,
	.cur_freq	= FIO_HZ_ONCE,
	.resp		= true,
	.len		= FIOMAN_FRAME_NO_80_SIZE
};

/* Frame 81: Switch Pack Drivers/CMU Status Request */
static FIOMSG_TX_FRAME frame_81_init =
{
	.def_freq	= FIO_HZ_10,
	.cur_freq	= FIO_HZ_10,
	.tx_func	= fioman_tx_frame_81,
	.resp		= true,
	.len		= FIOMAN_FRAME_NO_81_SIZE
};

/* Frame 82: CMU Datakey Read Request */
static FIOMSG_TX_FRAME frame_82_init =
{
	.def_freq	= FIO_HZ_ONCE,
	.cur_freq	= FIO_HZ_ONCE,
	.resp		= true,
	.len		= FIOMAN_FRAME_NO_82_SIZE
};

/* Frame 83: Switch Pack Drivers/CMU Status Request */
static FIOMSG_TX_FRAME frame_83_init =
{
	.def_freq	= FIO_HZ_10,
	.cur_freq	= FIO_HZ_10,
	.tx_func	= fioman_tx_frame_83,
	.resp		= true,
	.len		= FIOMAN_FRAME_NO_83_SIZE
};
...
/*****************************************************************************/
/*
This function is used to ready request frame 80 to be placed into the
request frame queue for transmission, for one port
*/
/*****************************************************************************/

void *
fioman_ready_frame_80
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 80 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_TX_FRAME */
	if ( ( p_tx = (FIOMSG_TX_FRAME *)kmalloc( sizeof( FIOMSG_TX_FRAME ) - 1
			+ FIOMAN_FRAME_NO_80_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		memcpy( p_tx, &frame_80_init, sizeof( frame_80_init ) );
		FIOMSG_PAYLOAD( p_tx )->frame_addr = device_to_addr(p_sys_fiod->fiod.fiod);
		FIOMSG_PAYLOAD( p_tx )->frame_ctrl = 0x83;
		FIOMSG_PAYLOAD( p_tx )->frame_no = FIOMAN_FRAME_NO_80;

		INIT_LIST_HEAD( &p_tx->elem );
		p_tx->when = FIOMSG_CURRENT_TIME;		/* Set when to send frame */
		p_tx->fioman_context = (void *)p_sys_fiod;
		p_tx->fiod = p_sys_fiod->fiod;
                if (p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_80] == -1)
		p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_80] = p_tx->cur_freq;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is used to ready request frame 81 to be placed into the
request frame queue for transmission, for one port
*/
/*****************************************************************************/

void *
fioman_ready_frame_81
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 61 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_TX_FRAME */
	if ( ( p_tx = (FIOMSG_TX_FRAME *)kmalloc( sizeof( FIOMSG_TX_FRAME ) - 1	+ FIOMAN_FRAME_NO_81_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		memcpy( p_tx, &frame_61_init, sizeof( frame_81_init ) );
		FIOMSG_PAYLOAD( p_tx )->frame_addr = 15;
		FIOMSG_PAYLOAD( p_tx )->frame_ctrl = 0x83;
		FIOMSG_PAYLOAD( p_tx )->frame_no = FIOMAN_FRAME_NO_81;
		INIT_LIST_HEAD( &p_tx->elem );
		p_tx->when = FIOMSG_CURRENT_TIME;		/* Set when to send frame */
		p_tx->fioman_context = (void *)p_sys_fiod;
		p_tx->fiod = p_sys_fiod->fiod;
                if (p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_81] == -1)
			p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_81] = p_tx->cur_freq;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is called when a frame 81 is to be TX'ed.
*/
/*****************************************************************************/
...
)
{
	...
	unsigned long flags;
/* TEG DEL */
/*pr_debug( "UPDATING Frame 81\n" );*/
/* TEG DEL */
	/* Fill in payload with channel status */
	memset(FIOMSG_PAYLOAD(p_tx_frame)->frame_info, 0, (FIOMAN_FRAME_NO_81_SIZE-3));
	list_for_each_safe( p_sys_elem, p_next, &fioman_fiod_list )
	{
		/* Get a ptr to this list entry */
		p_sys_fiod = list_entry( p_sys_elem, FIOMAN_SYS_FIOD, elem );
	...
	FIOMSG_PAYLOAD(p_tx_frame)->frame_info[12] = p_sys_fiod->cmu_mask;
	spin_unlock_irqrestore(&p_sys_fiod->lock, flags);
/*	printk( KERN_ALERT "UPDATING Frame 81: %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
			p_tx_frame->frame[3], p_tx_frame->frame[4], p_tx_frame->frame[5],
			p_tx_frame->frame[6], p_tx_frame->frame[7], p_tx_frame->frame[8],
			p_tx_frame->frame[9], p_tx_frame->frame[10], p_tx_frame->frame[11],
			p_tx_frame->frame[12], p_tx_frame->frame[13], p_tx_frame->frame[14],
...
/*****************************************************************************/
/*
This function is used to ready request frame 82 to be placed into the
request frame queue for transmission, for one port
*/
/*****************************************************************************/

void *
fioman_ready_frame_82
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 82 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_TX_FRAME */
	if ( ( p_tx = (FIOMSG_TX_FRAME *)kmalloc( sizeof( FIOMSG_TX_FRAME ) - 1
			+ FIOMAN_FRAME_NO_82_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		memcpy( p_tx, &frame_82_init, sizeof( frame_82_init ) );
		FIOMSG_PAYLOAD( p_tx )->frame_addr = device_to_addr(p_sys_fiod->fiod.fiod);
		FIOMSG_PAYLOAD( p_tx )->frame_ctrl = 0x83;
		FIOMSG_PAYLOAD( p_tx )->frame_no = FIOMAN_FRAME_NO_82;

		INIT_LIST_HEAD( &p_tx->elem );
		p_tx->when = FIOMSG_CURRENT_TIME;		/* Set when to send frame */
		p_tx->fioman_context = (void *)p_sys_fiod;
		p_tx->fiod = p_sys_fiod->fiod;
                if (p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_82] == -1)
		p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_82] = p_tx->cur_freq;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is called when a frame 82 is to be TX'ed.
*/
/*****************************************************************************/
...
/*****************************************************************************/
/*
This function is used to ready request frame 83 to be placed into the
request frame queue for transmission, for one port
*/
/*****************************************************************************/

void *
fioman_ready_frame_83
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 83 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_TX_FRAME */
	if ( ( p_tx = (FIOMSG_TX_FRAME *)kmalloc( sizeof( FIOMSG_TX_FRAME ) - 1	+ FIOMAN_FRAME_NO_83_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		memcpy( p_tx, &frame_83_init, sizeof( frame_83_init ) );
		FIOMSG_PAYLOAD( p_tx )->frame_addr = 15;
		FIOMSG_PAYLOAD( p_tx )->frame_ctrl = 0x83;
		FIOMSG_PAYLOAD( p_tx )->frame_no = FIOMAN_FRAME_NO_83;
		INIT_LIST_HEAD( &p_tx->elem );
		p_tx->when = FIOMSG_CURRENT_TIME;		/* Set when to send frame */
		p_tx->fioman_context = (void *)p_sys_fiod;
		p_tx->fiod = p_sys_fiod->fiod;
                if (p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_83] == -1)
			p_sys_fiod->frame_frequency_table[FIOMAN_FRAME_NO_83] = p_tx->cur_freq;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is called when a frame 83 is to be TX'ed.
*/
/*****************************************************************************/
...
)
{
	...
	unsigned long flags;
/* TEG DEL */
/*pr_debug( "UPDATING Frame 83\n" );*/
/* TEG DEL */
	/* Fill in payload with channel status */
	memset(FIOMSG_PAYLOAD(p_tx_frame)->frame_info, 0, (FIOMAN_FRAME_NO_83_SIZE-3));
	list_for_each_safe( p_sys_elem, p_next, &fioman_fiod_list )
	{
		/* Get a ptr to this list entry */
		p_sys_fiod = list_entry( p_sys_elem, FIOMAN_SYS_FIOD, elem );
	...
	FIOMSG_PAYLOAD(p_tx_frame)->frame_info[12] = p_sys_fiod->cmu_mask;
	spin_unlock_irqrestore(&p_sys_fiod->lock, flags);
/*	printk( KERN_ALERT "UPDATING Frame 83: %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
			p_tx_frame->frame[3], p_tx_frame->frame[4], p_tx_frame->frame[5],
			p_tx_frame->frame[6], p_tx_frame->frame[7], p_tx_frame->frame[8],
			p_tx_frame->frame[9], p_tx_frame->frame[10], p_tx_frame->frame[11],
			p_tx_frame->frame[12], p_tx_frame->frame[13], p_tx_frame->frame[14],
...
/* TEG DEL */
	...
	list_for_each_safe( p_sys_elem, p_next, &fioman_fiod_list )
	{
		...
		if (IS_TFBIU(p_sys_fiod->fiod.fiod)
			...
			|| (p_sys_fiod->fiod.fiod == FIO332)
                        || IS_OUTSIU(p_sys_fiod->fiod.fiod)) {
/*				pr_debug("Frame 0: out+: %x %x %x\n",	p_sys_fiod->outputs_plus[0],
					p_sys_fiod->outputs_plus[1], p_sys_fiod->outputs_plus[2] );
				pr_debug("Frame 0: out-: %x %x %x\n",	p_sys_fiod->outputs_minus[0],
					p_sys_fiod->outputs_minus[1], p_sys_fiod->outputs_minus[2] );
...
/*****************************************************************************/
/*
This function is used to ready response frame 208 to be placed into the
response frame list for this fiod for this port.
*/
/*****************************************************************************/

void *
fioman_ready_frame_208
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 208 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_RX_FRAME */
	if ( ( p_rx = (FIOMSG_RX_FRAME *)kzalloc( sizeof( FIOMSG_RX_FRAME ) - 1
		+ FIOMAN_FRAME_NO_208_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		fioman_rx_frame_init(p_sys_fiod, p_rx);
		FIOMSG_PAYLOAD( p_rx )->frame_no = FIOMAN_FRAME_NO_208;
		p_rx->len = FIOMAN_FRAME_NO_208_SIZE;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is used to ready response frame 209 to be placed into the
response frame list for this fiod for this port.
*/
/*****************************************************************************/

void *
fioman_ready_frame_209
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 209 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_RX_FRAME */
	if ( ( p_rx = (FIOMSG_RX_FRAME *)kzalloc( sizeof( FIOMSG_RX_FRAME ) - 1
		+ FIOMAN_FRAME_NO_209_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		fioman_rx_frame_init(p_sys_fiod, p_rx);
		FIOMSG_PAYLOAD( p_rx )->frame_no = FIOMAN_FRAME_NO_209;
		p_rx->rx_func = &fioman_rx_frame_209;
		p_rx->len = FIOMAN_FRAME_NO_209_SIZE;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is used to ready response frame 210 to be placed into the
response frame list for this fiod for this port.
*/
/*****************************************************************************/

void *
fioman_ready_frame_210
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* -1 is for the one byte of frame payload defined in FIOMSG_RX_FRAME */
	if ( ( p_rx = (FIOMSG_RX_FRAME *)kzalloc( sizeof( FIOMSG_RX_FRAME ) - 1
		+ FIOMAN_FRAME_NO_210_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		fioman_rx_frame_init(p_sys_fiod, p_rx);
		FIOMSG_PAYLOAD( p_rx )->frame_no = FIOMAN_FRAME_NO_210;
		p_rx->len = FIOMAN_FRAME_NO_210_SIZE;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is used to ready response frame 211 to be placed into the
response frame list for this fiod for this port.
*/
/*****************************************************************************/

void *
fioman_ready_frame_211
(
	FIOMAN_SYS_FIOD	*p_sys_fiod		/* FIOD of destined frame */
)
{
	...
	/* kalloc the actual frame 211 for this port */
	/* -1 is for the one byte of frame payload defined in FIOMSG_RX_FRAME */
	if ( ( p_rx = (FIOMSG_RX_FRAME *)kzalloc( sizeof( FIOMSG_RX_FRAME ) - 1
		+ FIOMAN_FRAME_NO_211_SIZE, GFP_KERNEL ) ) )
	{
		/* kmalloc succeeded, therefore init buffer */
		fioman_rx_frame_init(p_sys_fiod, p_rx);
		FIOMSG_PAYLOAD( p_rx )->frame_no = FIOMAN_FRAME_NO_211;
		p_rx->rx_func = &fioman_rx_frame_211;
		p_rx->len = FIOMAN_FRAME_NO_211_SIZE;
	}
	else
	{
		/* Could not kalloc */
...
/*****************************************************************************/
/*
This function is called when a frame 209 is RX'ed.
*/
/*****************************************************************************/
...
/*****************************************************************************/
/*
This function is called when a frame 211 is RX'ed.
*/
/*****************************************************************************/
